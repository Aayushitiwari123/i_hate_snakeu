...
def iHateSnakeu():
    ...
    foodx = round(random.randrange(1, (display_width - snake_block) + 2) / 10.0) * 10.0
    foody = round(random.randrange(1, (display_width - snake_block) + 2)/ 10.0) * 10.0

    while not game_over:
        while game_end == True:
            dis.fill(bluecolor)
            font_style = pygame.font.SysFont("comicsansms", 25)
            mesg = font_style.render("Wanna Play again? Press P", True, redcolor)
            ...
            for event in pygame.event.get():
                ...
                if event.type == pygame.QUIT:
                    game_over = True  # game window open
                    game_end = False  # game in ended
        for event in pygame.event.get():
            if event.type == pygame.QUIT:
                game_over = True
            if event.type == pygame.KEYDOWN:
        ...
        if x1 >= display_width or x1 < 0 or y1 >= display_height or y1 < 0:
            game_end = True
        # update co-ordinates of snakeu changed position
        x1 += x1_change
        y1 += y1_change
        dis.fill(blackcolor)
